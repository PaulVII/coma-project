...
public abstract class MatchResult {
	...
	/**
	 * smallest similarity value that is valid, indicates that objects are
	 * dissimilar
	 */
	...
	/**
	 * all match result operations that needs matchresults to have the same graphs
	 * (src, trg)
	 */
	...
	public static int stringToComposition(String composition) {
		...
		if (composition.equals("com_min"))
			return COMPOSITION_MIN;
		else if (composition.equals("com_max"))
			return COMPOSITION_MAX;
		else if (composition.startsWith("com_average"))
			return COMPOSITION_AVERAGE;
		return COMPOSITION_UNDEF;
	}
	...
	public ArrayList<Object> getSrcObjects() {
		return srcObjects;
	}

	public ArrayList<Object> getTrgObjects() {
		return trgObjects;
	}
	...
	public static int stringToOperation(String operation) {
		if (operation == null)
			return OP_UNDEF;
		else if (operation.startsWith("INTERSECT"))
			return OP_INTERSECT;
		else if (operation.startsWith("MERGE"))
			return OP_MERGE;
		else if (operation.startsWith("DIFF"))
			return OP_DIFF;
		else if (operation.startsWith("COMPOSE"))
			return OP_COMPOSE;
		else if (operation.startsWith("TRANSPOSE"))
			return OP_TRANSPOSE;
	...
	/**
	 * compare this mapping (the intended mapping) with another mapping (the test
	 * mapping)
	 * 
	 * @param testResult
	 ...
	 *         this function calls operations that need two matchresults as intput
	 *         e.g. intersect, compose
	 *         - if the matchresults are of the same kind (db, array) than call
	 *         their optimized methods
	 */
	 ...
	 * @param result
	 * @return result matchresult that is generated by the operation
	 *         this function calls operations that are based on one matchresult e.g.
	 *         transpose
	 */
	...
	public static MatchResult diff(MatchResult result1, MatchResult result2) {
		...
		// two different kinds of matchresult -> one is database (because only two kinds
		// exist)
	...
	public static boolean sameObjects(MatchResult[] results) {
		...
		for (int i = 0; i < results.length - 1; i++) {
			if (results[i].getSrcObjects() != null && results[i].getTrgObjects() != null &&
					(!results[i].getSrcObjects().equals(results[i + 1].getSrcObjects())
							|| !results[i].getTrgObjects().equals(results[i + 1].getTrgObjects()))) {
				sameObjects = false;
				break;
			}
		}
	 ...
	 * @param rightResult
	 * @return matchresult
	 *         return null if either of the matchresults is null or don't have a
	 *         same graph
	 */
	public static MatchResult compose(MatchResult leftResult, MatchResult rightResult, int composition) {
		...
		if (leftResult instanceof MatchResultArray && rightResult instanceof MatchResultArray) {
			...
			// } else if (leftResult instanceof MatchResultDB && rightResult instanceof
			// MatchResultDB){
			// return MatchResultDB.compose((MatchResultDB)leftResult,
			// (MatchResultDB)rightResult);
		}
		...
		if (verbose) {
			...
			System.out.println("LeftMapping: " + leftResult.getSourceGraph().getSource() + "<->"
					+ leftResult.getTargetGraph().getSource());
			System.out.println("RightMapping: " + rightResult.getSourceGraph().getSource() + "<->"
							...
							if (rightSim > 0) {
								...
								switch (composition) {
									...
									case COMPOSITION_MIN:
										sim = (leftSim < rightSim) ? leftSim : rightSim;
										break;
									case COMPOSITION_SUM: // used to later calculate correct average if more than two
															// matchresults
	...
	public static MatchResult transformMatchResult(MatchResult matchResult, int preprocessing) {
		...
		if (sourceGraph.getGraph(preprocessing) == null || targetGraph.getGraph(preprocessing) == null) {
			System.out.println("transformMatchResult(): one or both graphs do not support this targetState ("
					+ Graph.preprocessingToString(preprocessing) + ")!");
			return null;
		}
	...
	/**
	 * create a new match result which contains the correspondences of the given
	 * match result
		...
		for (int i = 0; i < srcObjects.size(); i++) {
			...
			for (int j = 0; j < trgObjects.size(); j++) {
				...
				if (sim > 0) {
					...
					String trgName;
					if (srcObject instanceof Path && trgObject instanceof Path) {
						srcName = ((Path) srcObject).toNameString();
						trgName = ((Path) trgObject).toNameString();
					} else {
						srcName = srcObject.toString();
						trgName = trgObject.toString();
					}
					sb.append("{\"source\":\"")
							.append(srcName)
							.append("\",\"target\":\"")
			...
			for (int j = 0; j < trgObjects.size(); j++) {
				...
				if (sim > 0) {
					try {
						...
						System.err.println("MatchResult.toConnectedGraph Error building weightedGraph");
					}
				}
			}
		}
	...
	public String getStatInfo() {
		// - SchemaMapping Statistics ( all src/trg Elements, # src/trg Elements with
		// Korresp., # all Korresp, # 1:1 Korrespondenzen (each element just has one
		// correspondence))
		StringBuffer buffer = new StringBuffer();
		buffer.append("Mapping: " + this.getName());
		buffer.append("\n\nSource: " + this.getSourceGraph().getSource().getName()
				+ "\tnodes: " + this.getSourceGraph().getElementCount() + "\tpaths: "
				+ this.getSourceGraph().getAllPaths().size());
		buffer.append("\nTarget:\t" + this.getTargetGraph().getSource().getName()
				+ "\tnodes: " + this.getTargetGraph().getElementCount() + "\tpaths: "
				+ this.getTargetGraph().getAllPaths().size());
		int[] matchC = getMatchCountAll();
		buffer.append("\n\nnumber of correspondences (count as 1:1): \t" + matchC[0]);
		buffer.append("\n\t\twith similarity value 1.0: \t\t" + matchC[1]);
		// buffer.append("\nnumber of correspondences (allowing n:m):\t" +
		// getMultipleMatches().size());
			...
			for (int j = 0; j < bObjects.size(); j++) {
				...
				if (sim > 0) {
					...
					// && !acc1.contains("/")&& !acc2.contains("/")
						...
						// acc1 = acc1.replaceAll("&", "%26").replaceAll("\'", "%27").replaceAll(",",
						// "%2C");
						// acc1 = acc1.replaceAll(":", "%3A").replaceAll("#", "%23");
						// while (acc1.indexOf('(') > 0 || acc1.indexOf(')') > 0) {
						// acc1 = acc1.replace("(", "%28").replace(")", "%29");
						// }
						// acc2 = acc2.replaceAll("&", "%26").replaceAll("\'", "%27").replaceAll(",",
						// "%2C").replaceAll(":", "%3A").replaceAll("#", "%23");
	 ...
	 *         similarity value is between 0 and 1 (0 indicate not similar, 1
	 *         indicates equal)
	 ...
	 * @param srcObject source object
	 * @param trgObject target object
	 *                  remove (if existing) similarity of the correspondence
	 *                  between both objects
	 */
	public abstract void remove(Object srcObject, Object trgObject);

	/**
	 * append the correspondence between two objects with its similarity to this
	 * match result
	 * 
	 * @param srcObject source object
	 * @param trgObject target object

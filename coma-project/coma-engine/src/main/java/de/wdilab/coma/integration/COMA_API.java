...
public class COMA_API {
	...
	// function: given two uri and match workflow, load and match
	// schemas/ontologies, return match result

	/**
	 * @param source            source file/url
	 * @param target            target file/url
	 * @param resolution        expected Resolution.RES2_XXX (currently SELFNODE,
	 *                          PARENTS, CHILDREN, LEAVES, SIBLINGS, SELFPATH)
	 * @param similaritymeasure SimilarityMeasure (should start with SIM_STR_ or
	 *                          SIM_DOC_ )
	 * @return match result
	 *         function: given two uri, load and match schemas/ontologies
	 *         (default/automatic configuration), return match result
	 */
	 ...
	 */
	public MatchResult matchModelsDefault(String fileSrc, String fileTrg, String fileAbbreviations,
			String fileSynonyms) {
		Graph graphSrc = loadGraph(fileSrc, null);
		Graph graphTrg = loadGraph(fileTrg, null);
	 ...
	 * @return graph tree
	 *         function: given a uri, transform schema/ontology to internal
	 *         structure, return tree
	 */
